...
Can Body Weight(kg) be determined by Workout Frequency, Workout Type, Calories Burned per Workout, and Session Duration of the Workout?
...
rq_1 <- subset(data, select = c(Weight, Workout_Frequency, Workout_Type, Calories_Burned, Session_Duration))
ggpairs(rq_1, lower = list(combo = "box_no_facet"))
```
```{r}
# Visualizing Research Question 1
rq_2 <- subset(data, select = c(Weight, Workout_Frequency, Workout_Type, Calories_Burned, Session_Duration))
ggpairs(rq_1, lower = list(combo = "box_no_facet"))
```
