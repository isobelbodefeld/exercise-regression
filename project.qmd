...
```{r}
interaction_model <- lm(Weight ~  Workout_Frequency + Workout_Type + Calories_Burned + Height + Avg_BPM * Fat_Percentage, data = data)

summary(interaction_model)
```
When Avg BPM increases by 1 unit, on average the effect that increasing Fat Percentage by 1 unit has on Body Weight decreases by 0.000633 kg.
